...
- **Whitespace smuggling hardening in `ImageTagBuilder::makeRelativeSrc()`** (CWE-20 / CWE-176, defence-in-depth) — a leading-whitespace payload like `" //evil.com/x.jpg"` previously bypassed the protocol-relative guard (the regex anchor `^//` cannot match past the space), passed through the leading-slash fallback as `/ //evil.com/x.jpg`, and would have been re-interpreted as a protocol-relative cross-origin reference once the browser trimmed the whitespace per WHATWG URL. ASCII whitespace is now stripped up-front. Not exploitable in the standard editor save path (the upstream `trim()` at `RteImageProcessor.php:120` covers it), but covered as defence-in-depth for server-side imports, third-party callers, and `UpdateImageReferences`. A `@security` docblock annotation records the escape-responsibility and path-canonicalisation contracts. See [ADR-004](https://docs.typo3.org/p/netresearch/rte-ckeditor-image/main/en-us/Architecture/ADR-004-Image-Src-Storage-Convention.html) for the full storage convention.
